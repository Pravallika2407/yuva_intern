...
| Script | Purpose |
|---|---|
| [`src/06_preprocessing_pipeline.py`](src/06_preprocessing_pipeline.py) | Full pipeline: load → deduplicate → handle missing values → flag outliers (IQR) → normalize units → scale features. Outputs `shipment_history_clean.csv`, used as input by the Week 1 modeling scripts. |

Report: `Week2_Data_Collection_Cleaning_Preprocessing_Report.docx` (submitted separately).
